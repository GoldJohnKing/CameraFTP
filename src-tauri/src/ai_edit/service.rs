...
impl AiEditService {
    ...
    async fn ensure_worker(&self) -> (mpsc::Sender<AiEditTask>, mpsc::Sender<AiEditTask>) {
        ...
        if needs_spawn {
            ...
            let (manual_sender, manual_receiver) =
                mpsc::channel::<AiEditTask>(MANUAL_QUEUE_CAPACITY);
    ...
    pub async fn on_file_uploaded(&self, file_path: PathBuf) {
        ...
        if let Err(e) = auto_sender.try_send(task) {
            ...
            warn!(
                ...
                dropped_task.file_path.display()
            );

            let file_name = dropped_task
                .file_path
                .file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("unknown")
            ...
            if let Err(emit_err) = self.app_handle.emit(
                "ai-edit-progress",
                &super::progress::AiEditProgressEvent::QueuedDropped {
                    file_name,
                    queue_depth: depth,
                },
            ) {
...
async fn worker_loop<R: tauri::Runtime>(
    ...
    fn emit_batch_done<R: tauri::Runtime>(
        ...
        if let Err(e) = app_handle.emit(
            "ai-edit-progress",
            &super::progress::AiEditProgressEvent::Done {
    ...
    loop {
        ...
        if let Err(e) = app_handle.emit(
            "ai-edit-progress",
            &super::progress::AiEditProgressEvent::Progress {
                current,
                total,
        ...
        match result {
            Some(Ok(ref output_path)) => {
                ...
                if let Err(e) = app_handle.emit(
                    "ai-edit-progress",
                    &super::progress::AiEditProgressEvent::Completed {
            ...
            Some(Err(ref e)) => {
                ...
                if let Err(e) = app_handle.emit(
                    "ai-edit-progress",
                    &super::progress::AiEditProgressEvent::Failed {
                        current: state.processed_count(),
                        total: state.processed_count() + remaining,
                        file_name: file_name.clone(),
                        error: e.to_string(),
...
async fn process_task(
    ...
    if cached_api_key.as_ref() != Some(&current_api_key)
        || cached_model.as_ref() != Some(&effective_model)
    {
    ...
    let prompt = task
        ...
        .as_deref()
        .or_else(|| {
            if ai_config.prompt.is_empty() {
                None
            } else {
                Some(&ai_config.prompt)
            }
        })
        .ok_or_else(|| AppError::AiEditError("提示词不能为空，请先配置提示词".to_string()))?;
    let image_bytes = provider
        .edit_image(&prepared.base64_data, prepared.mime_type, prompt)
        .await?;
...
async fn write_edited_image(
    ...
    if try_write_exclusive(&primary_path, image_bytes)
        .await
        .is_ok()
    {
...
mod tests {
    ...
    async fn write_edited_image_handles_collisions() {
        ...
        tokio::fs::write(&collision_path, b"existing")
            .await
            .unwrap();
    ...
    fn prompt_selection_prefers_override_over_config() {
        ...
        let result = override_prompt.or_else(|| {
    ...
    async fn select_next_task_prefers_manual_over_auto() {
        ...
        match select_next_task(&mut manual_rx, &mut auto_rx, &token).await {
            ...
            SelectOutcome::AutoChannelClosed(_) => {
                panic!("expected manual task, got AutoChannelClosed")
            }
    ...
    async fn select_next_task_returns_auto_when_manual_idle() {
        ...
        match select_next_task(&mut manual_rx, &mut auto_rx, &token).await {
            ...
            SelectOutcome::AutoChannelClosed(_) => {
                panic!("expected auto task, got AutoChannelClosed")
            }
    ...
    async fn select_next_task_cancelled_wins_over_ready_channels() {
        ...
        match select_next_task(&mut manual_rx, &mut auto_rx, &token).await {
            ...
            SelectOutcome::AutoChannelClosed(_) => {
                panic!("expected Cancelled, got AutoChannelClosed")
            }
    ...
    async fn select_next_task_returns_shutdown_when_both_channels_closed() {
        ...
        match select_next_task(&mut manual_rx, &mut auto_rx, &token).await {
            ...
            SelectOutcome::AutoChannelClosed(_) => {
                panic!("expected ShutDown, got AutoChannelClosed")
            }
        }
    }
    ...
    async fn select_next_task_falls_back_to_manual_when_auto_closed() {
        ...
        let select =
            tokio::spawn(
                async move { select_next_task(&mut manual_rx, &mut auto_rx, &token).await },
            );
    ...
    async fn worker_loop_processes_tasks_and_emits_done_when_queue_drains() {
        ...
        assert_eq!(
            done_count, 1,
            "exactly one non-cancelled Done(3 failed) expected: {:?}",
            ev
        );
        drop(ev);
    ...
    async fn worker_loop_cancel_drains_pending_tasks_and_recovers_with_fresh_token() {
        ...
        wait_until(Duration::from_secs(10), || {
            ...
            let done_cancelled = events.lock().unwrap().iter().any(|e| {
                matches!(
                    e,
                    AiEditProgressEvent::Done {
                        cancelled: true,
...

        {
            ...
            assert!(
                !ev.iter()
                    .any(|e| matches!(e, AiEditProgressEvent::Failed { .. })),
