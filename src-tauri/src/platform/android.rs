...
#[cfg(target_os = "android")]
const SYNC_NATIVE_PROCESSING_STATE_SIGNATURE: &str = "(Landroid/content/Context;Z)V";
...
pub fn sync_processing_state(active: bool) {
    use std::sync::atomic::Ordering;
    match tokio::runtime::Handle::try_current() {
        Ok(handle) => {
            ...
            handle.spawn_blocking(move || {
                ...
                if let Err(error) = sync_android_processing_state(active) {
...
#[cfg(target_os = "android")]
fn sync_android_processing_state(active: bool) -> Result<(), String> {
    use crate::error::AppError;
