...
import { createTaskProgressService } from './task-progress';
import type { TaskProgressState } from './task-progress';
...
export function mapToState(state: TaskProgressState): ColorGradingProgressState {
  return { ...state, isProcessing: state.isActive };
}

const colorGrading = createTaskProgressService<ColorGradingEvent>({
...
export const colorGradingStore = colorGrading.store;
export const ensureColorGradingListener = () => colorGrading.ensureListener();
