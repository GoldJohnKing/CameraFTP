...
export interface TaskProgressServiceConfig<TEvent extends { type: string }> {
  eventName: string;
  debugLabel: string;
...
export interface TaskProgressService {
  /** Vanilla zustand store; subscribe from React via `useStore(store)`. */
  store: StoreApi<TaskProgressState>;
  /** Idempotently register the Tauri event listener. */
  ensureListener: () => void;
  dismissDone: () => void;
  getProgressState: () => TaskProgressState;
}

export function createTaskProgressService<TEvent extends { type: string }>(
  config: TaskProgressServiceConfig<TEvent>,
): TaskProgressService {
  const store = createStore<TaskProgressState>(() => ({ ...initialTaskProgressState }));
  ...
  return { store, ensureListener, dismissDone, getProgressState };
}
